// ========================================================================
// CORRECTED ALU Reference Model  FINAL FIXED VERSION
// ========================================================================
...
class alu_reference_model;
  ...
  function alu_packet compute(alu_packet in_pkt);
    ...
    // ---------- DECLARATIONS AT TOP (Questa Safe) ----------
    int rot_amt;
    logic [15:0] mul_tmp;
    ...
    if (in_pkt.mode == 1) begin
      ...
      case (in_pkt.cmd)
        ...
        4'h0: begin // ADD
          ...
          exp_pkt.oflow = (in_pkt.opa[7] == in_pkt.opb[7]) &&
                          (exp_pkt.res[7] != in_pkt.opa[7]);
        end

        4'h1: begin // SUB
          exp_pkt.res   = {1'b0, in_pkt.opa} - {1'b0, in_pkt.opb};
          exp_pkt.cout  = (in_pkt.opa < in_pkt.opb);
          exp_pkt.oflow = (in_pkt.opa[7] != in_pkt.opb[7]) &&
                          (exp_pkt.res[7] != in_pkt.opa[7]);
        end
        ...
        4'h3: begin // SUB_CIN
          exp_pkt.res   = {1'b0, in_pkt.opa} - {1'b0, in_pkt.opb} - in_pkt.cin;
          exp_pkt.cout  = (in_pkt.opa < (in_pkt.opb + in_pkt.cin));
        end

        4'h4: begin // INC_A
          exp_pkt.res  = {1'b0, in_pkt.opa} + 1;
          exp_pkt.cout = (in_pkt.opa == 8'hFF);
        end

        4'h5: begin // DEC_A
          exp_pkt.res  = {1'b0, in_pkt.opa} - 1;
          exp_pkt.cout = (in_pkt.opa == 8'h00);
        end

        4'h6: begin // INC_B
          exp_pkt.res  = {1'b0, in_pkt.opb} + 1;
          exp_pkt.cout = (in_pkt.opb == 8'hFF);
        end

        4'h7: begin // DEC_B
          exp_pkt.res  = {1'b0, in_pkt.opb} - 1;
          exp_pkt.cout = (in_pkt.opb == 8'h00);
        end
